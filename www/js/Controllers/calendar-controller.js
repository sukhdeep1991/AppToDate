...
.controller('calendarCtrl', function($scope, $filter, $location, eventService) {
 ...
 if($scope.userDetails && $scope.userDetails.user_id){
	 eventService.getEvents($scope.userDetails.user_id).then(function(data){
		 console.log("Events fetched : " + JSON.stringify(data));
		 $scope.events = data;
		...
		console.log("Error at controller while fetching events"); 
	 });
 } else {
	 var date = new Date();
	    var d = date.getDate();
