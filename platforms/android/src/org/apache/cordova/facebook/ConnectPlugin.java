			...
			Request.newMeRequest(session, new Request.GraphUserCallback() {
				...
				public void onCompleted(GraphUser user, Response response) {
					...
					if (loginContext != null) {
						...
						try{
							jsonResponse.put("user", new JSONObject(graphObject.getInnerJSONObject().toString()));
						}catch(JSONException e){
							Log.d(TAG, "Error adding login object " + e.getLocalizedMessage());
						}
						loginContext.success(jsonResponse);
						loginContext = null;
					}
				}
			}).executeAsync();
